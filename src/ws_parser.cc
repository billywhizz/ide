        ...
        switch(parser->index) {
          ...
          case 2:
            if(parser->payload16 == 1) {
              ...
              current->maskkey[0] = p[start] & 0xff;
            }
            break;
          case 3:
            if(parser->payload16 == 1) {
              ...
              current->maskkey[1] = p[start] & 0xff;
            }
            break;
          case 4:
            if(parser->payload16 == 1) {
              ...
              current->maskkey[2] = p[start] & 0xff;
            }
            break;
          case 5:
            if(parser->payload16 == 1) {
              ...
              current->maskkey[3] = p[start] & 0xff;
              if(current->length > 0) {
                parser->state = WS_BODY;
                parser->bodypos = 0;
                settings->on_header(parser);
      ...
      case WS_BODY:
        if(current->mask == 1) {
          ...
          if(toread == 0) {
            ...
            while(done--) {
              p[currpos] = (p[currpos] & 0xff) ^ current->maskkey[parser->maskpos++];
              if(parser->maskpos == 4) {
                parser->maskpos = 0;
              }
              currpos++;
            }
            const char* b = data + start;
            settings->on_chunk(parser, b, toread);
            settings->on_complete(parser);
            parser->index = 0;
            parser->state = WS_HEADER;
            start = currpos;
